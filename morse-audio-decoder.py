...
DIRS = [SCRIPT_DIR, PROFILE_DIR, EXPORT_DIR]
...
CHAR_SPACE = '(char)'  # 3 * DIT of 0 values
WORD_SPACE = '(space)'
SYMBOL_SPACE = ''
...
class MorseDecoder():
    ...
    def translate_to_binary(self):
        myprint("> Translating {os.path.basename(self.audio_file)}.", 2)
        binary = []
    ...
    def translate_length(self, length, value):
        ...
        if value == 1:
            ...
            if length > SPACE - (SPACE * thresh):
                myprint(f"{length}ms of {value} > {SPACE - (SPACE * thresh)} | Word space", 1)
                self.decoded_audio.append(WORD_SPACE)
            elif length > DAH - (DAH * thresh):
                myprint(f"{length}ms of {value} | Character space", 1)
